      ...
      <div className="mt-4 ">
        {activeTab === "description" && (
          <div className=" bg-secondary p-4">
            ...
            <ul className="list-disc pl-5 text-paragraph">
        ...
        {activeTab === "review" && (
          <div className="flex flex-col md:flex-row  gap-4 w-full  ">
            ...
            <div className="w-full md:w-1/2  p-6 shadow-md rounded-md  bg-secondary">
              <h3 className=" font-bold text-heading ">4.00</h3>
              ...
              <hr className="my-3" />
              <h3 className=" font-semibold text-heading">Review this product</h3>
              ...
              {ratings.map((rating) => (
                <div
                  key={rating.stars}
                  className="flex items-center space-x-2 my-1 text-heading"
                >
                  ...
                  <div className="flex-start flex h-2.5 w-full overflow-hidden rounded-full bg-primary font-sans text-xs font-medium">
                    <div
                      className="flex h-full items-center justify-center overflow-hidden break-all rounded-full bg-mainText text-white"
                      style={{
                        width: `${(rating.count / totalRatings) * 100}%`,
                      }}
                    ></div>
                  </div>
                  <span className="text-sm text-heading">{rating.count}</span>
                </div>
              ))}

              <button className="w-full mt-32 bg-mainText text-white py-2 font-semibold rounded-md hover:bg-opacity-90">
                Write Review
              </button>
            </div>

            {/* Right Section - Reviews List */}
            <div className="w-full md:w-1/2 max-h-[500px] overflow-y-auto  bg-secondary rounded-lg p-4">
              {reviews.map((review) => (
                <div
                  key={review.id}
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-heading">{review.date}</p>
                      </div>
                    </div>
                    <div className="flex">
                      {Array.from({ length: 5 }, (_, index) => (
                        <span
                          key={index}
                          className={`text-yellow-400  ${index < review.rating ? "opacity-100" : "opacity-50"
                            }`}
                        >
                          ★
                        </span>
                  ...
                  <p className="text-paragraph mt-2">{review.review}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeTab === "qa" && (
          <div className="p-6 bg-secondary">
            <div className="flex justify-between items-center mb-4">
              ...
              <button className="bg-mainText text-white px-4 py-2 text-sm font-semibold rounded-md hover:bg-gray-300">
                Post Your Question
              </button>
            </div>
              ...
              {questions.map((qa) => (
                ...
                >
                  ...
                  <div className="flex items-start">
                    <span className="bg-mainText text-white px-2 py-1 text-sm font-semibold rounded-md">
                      {qa.answerId}
                    </span>
                    <p className="ml-2 text-paragraph">{qa.answer}</p>
                  </div>
