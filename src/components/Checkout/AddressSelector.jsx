...
const AddressSelector = ({
  ...
  const {
    containerStyle = "bg-secondary p-6 rounded-lg shadow-lg",
    titleStyle = "tracking-primary text-heading ",
    addButtonStyle = "text-heading font-bold flex items-center hover:underline",
    addressBoxStyle = "cursor-pointer  p-4 rounded-lg transition-all ",
    selectedBoxStyle = " text-mainText shadow-md ",
    unselectedBoxStyle = " text-heading",
            ...
            <div className="flex items-start">
              {showIcons && (
                <div className="mr-3 mt-1">
                  {selectedAddress === address.id ? (
                    <IoIosRadioButtonOn
                      className={`text-mainText ${radioIconSize}`}
                    />
                  ) : (
                    <div className="w-4 h-4 border  text-heading rounded-full"></div>
                  )}
                </div>
              )}
              <div>
